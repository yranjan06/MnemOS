...
def _emit_node(
    ...
    if node.type == "Remember":
        ...
        lines.append(f"{pad}    run_id='global',")
        lines.append(f"{pad})")
    ...
    if node.type == "Recall":
        ...
        lines.append(f'{pad}report_node("{node.id}", "running")')
        lines.append(f'{pad}print("--- {node.id}: {node.label} ---")')
        lines.append(f"{pad}from memory import recall_all_runs as _mnemos_recall")
    ...
    if node.type == "Plan":
        ...
        lines.append(f"{pad}_{node.id}_task_text = {task_expr}")
        lines.append(f"{pad}_{node.id}_past = await _plan_recall(_{node.id}_task_text, _workflow_id, top_k=3)")
        ...
        lines.append(f"{pad}    f\"Task: {{_{node.id}_task_text}}\\n\"")
